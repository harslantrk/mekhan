        	...
        	<div class="ibox float-e-margins">
	            ...
	            <div class="ibox-content">
		            <table class="table table-striped table-bordered table-hover dataTables-js" >
			            ...
			            <tbody>
			            	<?php foreach($iletisim as  $s):?>
					            <tr class="gradeX">
					            	<td class="data-id"><?=$s["id"]?></td>
					                <td class="data-caption"><?=$s["ad"]?></td>
									<td class="data-caption"><?=$s["tel"]?></td>
									<td class="data-caption"><?=$s["email"]?></td>
									<td class="data-caption"><?=kisalt($s["mesaj"],20)?></td>
									<td class="data-caption"><?=$s["tarih"]?></td>
					                <td>
					                	<button rel="tooltip" title="İncele" onclick="location='<?=base_url()?>admin/iletisim/update_iletisim/<?=$s["id"]?>'" class="btn btn-primary btn-circle btn-outline " type="button"><i class="fa fa-search"></i></button>
					                	<button rel="tooltip" title="Aktif/Pasif" ajax-handler="iletisim_status" class="toggle-statu btn btn-default btn-circle btn-outline" type="button" ><i class="fa fa-<?php if($s["status"]==1){echo "check";}else{echo "uncheck";}?>"></i></button>
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content animated flipInY">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-warning" id="delete-item" delete-handler="iletisim_delete" delete-id="" data-dismiss="modal"> <i class="fa fa-trash"></i> Sil</button>
            </div>
        </div>
    </div>
</div>