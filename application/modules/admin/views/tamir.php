        	...
        	<div class="ibox float-e-margins">
	            <div class="ibox-title">
	                ...
	                <div class="ibox-tools">
	                    <a href="<?=base_url()?>admin/tamir/insert_tamir" class="btn btn-primary" style="margin: -10px -10px 0px 0px;"> <i class="fa fa-plus"></i> tamir Ekle</a>
	                </div>
	            </div>
	            <div class="ibox-content">
		            <table class="table table-striped table-bordered table-hover dataTables-js" >
			            ...
			            <tbody>
			            	<?php foreach($tamir as  $s):?>
					            <tr class="gradeX">
					            	<td class="data-id"><?=$s["id"]?></td>
					                <td class="data-caption"><?=$s["title"]?></td>
					                <td>
					                	<button rel="tooltip" title="Düzenle" onclick="location='<?=base_url()?>admin/tamir/update_tamir/<?=$s["id"]?>'" class="btn btn-primary btn-circle btn-outline " type="button"><i class="fa fa-pencil"></i></button>
					                	<button rel="tooltip" title="Aktif/Pasif" ajax-handler="tamir_status" class="toggle-statu btn btn-default btn-circle btn-outline" type="button" ><i class="fa fa-<?php if($s["status"]==1){echo "check";}else{echo "uncheck";}?>"></i></button>
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content animated flipInY">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-warning" id="delete-item" delete-handler="tamir_delete" delete-id="" data-dismiss="modal"> <i class="fa fa-trash"></i> Sil</button>
            </div>
        </div>
    </div>
</div>