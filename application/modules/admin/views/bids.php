        	...
        	<div class="ibox float-e-margins">
	            ...
	            <div class="ibox-content">
		            <div class="pull-right">
	                    <form method="post" id="list_form" action="<?=base_url('admin/bids/bid_list')?>"> 
	                    	<input type="hidden" name="form_control" id="form_control" value="1">     
	                      	...
	                      	<div class="col-xs-12 col-sm-5 col-md-5 col-lg-3">
	                        	<div class="input-group">
	                        		<input type="text" placeholder="Tarih Aralığı" name="datetime_bids" id="datetime_bids" value="<?=$this->session->userdata('datetime_bids')?>" class="input-sm form-control">
	                        	</div>
	                        </div>
	                        
	                        <div class="col-xs-12 col-sm-5 col-md-5 col-lg-4">
		                        <div class="input-group"><input type="text" placeholder="Kayıt" name="query_bids" id="query_bids" value="<?=$this->session->userdata('query_bids')?>" class="input-sm form-control"> 
			            ...
			            <tbody>
			            	<?php if($list){foreach($list as  $li):?>
					            <tr class="gradeX">
					                ...
					                <td>
					                	<?php if(array_search('update',$this->session->userdata('auth')['bids'])!==false){?>
					                		<button rel="tooltip" title="Düzenle" onclick="location='<?=base_url()?>admin/bids/update_bid/<?=$li['id']?>'" class="btn btn-primary btn-circle btn-outline " type="button"><i class="fa fa-pencil"></i></button>
					                	<?php } ?>
					                	<?php if(array_search('status',$this->session->userdata('auth')['bids'])!==false){?>
					                		<button rel="tooltip" title="Aktif/Pasif" ajax-handler="bid_status" class="toggle-statu btn btn-default btn-circle btn-outline" type="button" ><i class="fa fa-<?php if($li['status']==1){echo "check";}else{echo "uncheck";}?>"></i></button>
					                	<?php } ?>
					                	<?php if(array_search('delete',$this->session->userdata('auth')['bids'])!==false){?>
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content animated flipInY">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-warning" id="delete-item" delete-handler="bid_delete" delete-id="" data-dismiss="modal"> <i class="fa fa-trash"></i> Sil</button>
            </div>
        </div>
    </div>
</div>