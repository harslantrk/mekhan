					...
					<div class="row">
						...
						<div class="col-md-6">
							...
							<table class="table table-hover table-striped">
								<?php
								foreach($_SESSION["tamir"] as $key => $ozel){ 
								if($key == "id" || $key == "Fiyat"){
								}else{
								?>
									...
									<tr>
										...
										<td>: <?php if($ozel == 0){ echo "Yok";}else{ echo "<b style='color:red'>Var</b>";}; ?></td>
									</tr>
								<?php } }?>
							</table>
						</div>
						...
						<div class="col-md-12">
							<hr/>
							<h4>Fiyat : <?php echo $_SESSION["tamir"]["Fiyat"];?> TL</h4>
				...
				<div class="col-md-offset-1 col-md-10">
					<div class="col-md-12">
						<div class="row">
							...
							<?php } ?>
							<form action="<?php echo base_url("tamirsiparis/tamirsiparis/".$this->uri->segment(3,0)."/".$bilgiler["id"]); ?>" method="post">
								<input type="hidden"  	name="siparis" 	value="siparis" id="siparis"/>
								<input type="hidden"  	name="fiyat" 	value="<?php echo $_SESSION["tamir"]["Fiyat"];?>"/>
	...
	<script>
		$(function(){
			$("#il").change(function(){	
				...
				var url = "<?php echo base_url("tamirsiparis/tamirurunsiparis"); ?>";
				 $.ajax({
				   type: "POST",
				   url: url,
				   data: {il : il}, 
